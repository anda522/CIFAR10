...
# 获取每一个 img数据 和 label
# 训练数据集
train_data = torchvision.datasets.CIFAR10("./dataset", train=True, transform=transform_train,
                                          download=True)
test_data = torchvision.datasets.CIFAR10("./dataset", train=False, transform=transform_test,
                                         download=True)
...
for i in range(epoch):
    ...
    # 输出最大预测率的模型
    if total_accuracy / len(test_data) > max_acc:
        max_acc = total_accuracy / len(test_data)
        torch.save(model, "./logs/model{}.pth".format(i + 1))
    # 也可以方式二
        print("更优模型已保存")
